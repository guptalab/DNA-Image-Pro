...
2008 Changed tile and bond data type to short int, for > 256 types.  For smaller memory footprint, xgrow-small is avalailable. (CE)
   ...
   2/2013 Numerous fixes for double tiles with and without fission, as well as some update_rate speedups
          (removing obsolescent code). Also, make left mouse button identify and middle puncture. See
          hg log for complete changes. 
...
# define NSIZE 42
#ifdef SMALL
# define MAXTILETYPES 1048576 //Increase MAXTILETYPE for large Images (1024*1024)
#else
...
# define icon_bitmap_height 16
   ...
   for (i=1;i<MAXTILETYPES;i++) 
   {
      ...
      // Added by Dhaval Trivedi
      // Edit to use any pixel value. (Mainly for Image processing purpose)
     {
         translate[i]=atoi(tile_colors[i]);
         if(translate[i]==0 && XAllocNamedColor(display,cmap,tile_colors[i],&colorcell,&xcolor))
         {
